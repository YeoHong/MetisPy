import sys, os
from MetisPy import ROOT_DIR, __appname__
sys.path.append(os.path.abspath(ROOT_DIR)) # To find local version of the library
...
from MetisPy.canvas.widgets.canvas import Canvas
from MetisPy.canvas.widgets.label_dialog import LabelDialog
from MetisPy.canvas.widgets.label_list_widget import LabelListWidget
from MetisPy.canvas.widgets.label_list_widget import LabelListWidgetItem
...
from MetisPy.utils.label_file import LabelFileError

from MetisPy import utils
from MetisPy.config import get_config
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, config=None, filename=None, output=None, output_file=None, output_dir=None):
        ...
        self.updateFileMenu()
        # Since loading thie file may take some time,
        # maek sure it runs in the backgroud.
    ...
    def setScrollArea(self):
        ...
        self.scrollBars = {
            QtCore.Qt.Vertical: scrollArea.verticalScrollBar(),
            QtCore.Qt.Horizontal: scrollArea.horizontalScrollBar(),
        }
    ...
    def setMenums(self):
        ...
        # Group zoom controls into a list for eaiser toggling.
        zoomActions = (
            self.zoomWidget,
            zoomIn,
            zoomOut,
        ...
        fitWindow.setChecked(QtCore.Qt.Checked)
        self.scalers = {
        ...
        # Custom contex menu for the canvas widget:
    ...
    def newShape(self):
        ...
        text = None
        if items:
            text = items[0].data(QtCore.Qt.UserRole)
        flags = {}
        group_id = None
    ...
    def loadFile(self, filename=None):
        """Laod the specified file, or the last opened file if None."""
    ...
    def openPrevImg(self, _value=False):
        ...
        if QtCore.Qt.KeyboardModifiers() == (QtCore.Qt.ControlModifier | QtCore.Qt.ShiftModifier):
    ...
    def saveLabels(self, filename):
        ...
        for i in range(self.flag_widget.count()):
            ...
            key = item.text()
            flag = item.checkState() == QtCore.Qt.Checked
            flags[key] = flag
        try:
            ...
            items = self.fileListWidget.findItems(
                self.imagePath, QtCore.Qt.MatchExactly
            )
            if len(items) > 0:
                if len(items) != 1:
    ...
    def labelItemChanged(self, item):
        shape = item.shape()
        self.canvas.setShapeVisible(shape, item.checkState() == QtCore.Qt.Checked)
    ...
    def editLabel(self, item=None):
        ...
        if not self.uniqLabelList.findItemsByLabel(shape.label):
            ...
            item.setData(QtCore.Qt.UserRole, shape.label)
            self.uniqLabelList.addItem(item)
